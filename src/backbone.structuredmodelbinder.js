...
(function (factory) {
  ...
  Backbone.StructuredModelBinder = (function(_, $, Backbone){
    ...
    _.extend(ModelBinder, {
      ...
      //Functions
      bind: function(model, el, bindings, options){
        this.unbind();
        ...
        if (!bindings) throw 'bindings must be specified (for now)';

        if(bindings){
          // Create a deep clone of the attribute bindings
          this._bindings = $.extend(true, {}, bindings);
    ...
    _.extend(ModelBinder.Model, {
      ...
      */
      ...
      setObj: function(jsonModel, attrs, dotPath, options){
        console.log(attrs);
